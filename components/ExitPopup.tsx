...
const ExitPopup: React.FC<ExitPopupProps> = ({ onEmailSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Popup */}
      <div className="relative w-full max-w-lg bg-silk shadow-2xl animate-scale-in overflow-hidden">
        {/* Close button */}
        <button 
          onClick={handleClose}
          className="absolute top-4 right-4 z-10 p-2 text-gray-400 hover:text-charcoal transition-colors"
        >
          <X size={20} />
        </button>
        ...
        <div className="p-8 md:p-12">
          {status === 'success' ? (
            <div className="text-center py-8 animate-fade-in">
              ...
              <h3 className="text-2xl font-serif text-charcoal mb-3">Check je inbox!</h3>
              <p className="text-gray-600 font-light">
                De gids is onderweg naar {email}
              </p>
              <button 
                onClick={handleClose}
                className="mt-8 text-xs uppercase tracking-widest text-gold hover:text-charcoal transition-colors"
              >
                Sluiten
              </button>
            </div>
            ...
            <>
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-gold/10 text-gold text-xs uppercase tracking-widest mb-6">
                <Gift size={14} />
                Gratis Download
              </div>
              ...
              {/* What's inside */}
              <div className="bg-charcoal/5 p-6 mb-8">
                <span className="text-xs uppercase tracking-widest text-gray-400 mb-4 block">In deze gids:</span>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-4 bg-white border border-gray-200 text-charcoal placeholder-gray-400 focus:outline-none focus:border-gold transition-colors"
                  />
                </div>
                <button
                  type="submit"
                  disabled={status === 'loading'}
                  className="w-full group relative px-8 py-4 bg-gold text-charcoal text-xs uppercase tracking-[0.2em] font-medium transition-all duration-300 hover:bg-charcoal hover:text-white flex items-center justify-center gap-3 disabled:opacity-50"
                >
                  {status === 'loading' ? (
